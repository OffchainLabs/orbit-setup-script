...
async function main() {
  ...
  while (true) {
    ...
    if (newBalance.sub(oldBalance).gte(ethers.utils.parseEther(amount))) {
        console.log(`LFG! 🚀 Balance of your account on appchain increased by ${amount} Ether.`);
        break;
    }
    console.log("Balance not changed yet. Waiting for another 30 seconds to receive the funds on the appchain ⏰⏰⏰⏰⏰⏰");
    await delay(30 * 1000);
}
