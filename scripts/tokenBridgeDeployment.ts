...
export const deployErc20L3 = async (rs: RuntimeState,deployer: Signer): Promise<L3> => {  
  ...
  const beaconProxyFactory = rs.l3.beaconProxyFactory ?
      ...
      await deployContract(
          deployer, 
          new BeaconProxyFactory__factory()
      )
...
export async function tokenBridgeDeployment(privateKey: string, L2_RPC_URL: string, L3_RPC_URL: string, rs: RuntimeState) {
    ...
    const { l2, l3 } = await deployErc20AndInit(l2Signer, l3Signer, inboxAddress, rs);
